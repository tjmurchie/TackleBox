...
Features:
    ...
    - O-pool design: T7 promoter + amplification primer + BspQI cut site
      (following Hackenberger et al. 2024)
...
Oligo pool synthesis approach adapted from:
    Hackenberger et al. 2024. CARPDM: cost-effective antibiotic resistome
    profiling of metagenomic samples using targeted enrichment.
    bioRxiv 2024.03.27.587061.
...
def design_opool(baits: List[Bait], output_dir: str, prefix: str,
    ...
    """Design oligo pool with T7 promoter and amplification primer.

    Returns (oligo_pool_path, bare_probes_path, primer_fasta_path, primer_seq).

    Structure of final oligo:
    5'-GCTAATACGACTCACTATAGGG [80bp probe] GAAGAGC [rc_primer] -3'
       T7 promoter             probe        BspQI    amplification primer
    """
    T7_primer = 'GCTAATACGACTCACTATAGGG'

    # Write bare probes (self_filter equivalent)
    bare_probes_path = os.path.join(output_dir, f'{prefix}_probes.fna')
    write_fasta(bare_probes_path,
                [(b.bait_id, b.seq.upper()) for b in sorted(baits, key=lambda x: x.bait_id)])
    ...
    primer_length = 18
    first_nts = ''
    last_nts = 'GCTCTTCG'  # BspQI site + G clamp
    ...
    for seq in sorted(order_set):
        ...
        if any([
            ...
            'GGG' in primer_seq,
            'CCC' in primer_seq,
            'GAAGAGC' in primer_seq,
            'GCTCTTC' in primer_seq[:-2]
        ]):
            continue
    ...
    if len(candidate_records) == 0:
        log_fn("WARNING: No candidate primers found. Skipping o-pool design.")
        primer_fasta_path = os.path.join(output_dir, f'{prefix}_amplification_primers.fna')
        with open(primer_fasta_path, 'w') as f:
            f.write(f'>T7_primer\n{T7_primer}\n')
        oligo_path = bare_probes_path
        return oligo_path, bare_probes_path, primer_fasta_path, ''

    # Write candidate primers to temp file
    ...
    # Attach T7 to probes for BLAST comparison
    t7_probes_fa = os.path.join(output_dir, f'{prefix}_T7_probes_temp.fasta')
    t7_records = []
    for b in baits:
        rec = SeqRecord(Seq(T7_primer + b.seq.upper()), id=b.bait_id, description='')
        t7_records.append(rec)
    SeqIO.write(t7_records, t7_probes_fa, 'fasta')

    # BLAST primers against T7-appended probes
    header = 'qseqid sseqid length nident sstrand'.split()
    primer_blast_out = os.path.join(output_dir, f'{prefix}_primer_blast_temp.txt')
    outfmt = f'6 {" ".join(header)}'
    cmd = shlex.split(
        f'blastn -task "blastn-short" -query {cand_primer_fa} '
        f'-out {primer_blast_out} -outfmt "{outfmt}" '
        f'-subject {t7_probes_fa} -word_size 4 -evalue 50')
    subprocess.run(cmd, stderr=subprocess.DEVNULL)

    # Parse BLAST: select primer with fewest total identities
    try:
        primer_blast_df = pd.read_csv(primer_blast_out, sep='\t', names=header)
        id_dict = {}
        for p in primer_blast_df['qseqid'].unique():
            id_dict[p] = primer_blast_df.loc[
        ...
        log_fn("WARNING: Primer BLAST parsing failed. Using first candidate.")
        final_primer_id = candidate_records[0].id
...
def blast_validation(probe_fasta: str, design_fasta: str, output_dir: str,
                     ...
                     log_fn) -> None:
    """BLAST probes against original input for coverage analysis and plotting."""
    # BLAST probes against design targets
    blast_xml = os.path.join(output_dir, f'{prefix}_final_blast.xml')
    cmd = shlex.split(
        f'blastn -query {probe_fasta} -subject {design_fasta} '
        f'-out {blast_xml} -outfmt 5')
    subprocess.run(cmd, stderr=subprocess.DEVNULL)

    # Build coverage and count dicts
    coverage_dict = {}
    gc_dict = {}
    target_seq_dict = {}
    probe_count_dict = {}
            ...
            for alignment in record.alignments:
                for hsp in alignment.hsps:
                    if hsp.identities > identity_cutoff and not any([
                        re.search(r'--+', hsp.sbjct) is not None,
                        re.search(r'--+', hsp.query) is not None
                    ]):
                        target = alignment.title.split('|')[-1].split()[0]
                        if target in target_count_dict:
                        ...
                        if target in coverage_dict:
                            ...
                            if pad_right >= 0:
                                padded = np.pad(match_array,
                                                (pad_left, pad_right),
                                                mode='constant',
                                                constant_values=(0, 0))
                                coverage_dict[target] += padded

    # Write target:probe pairs
    ...
    for target in coverage_dict:
        ...
        info_list = [
            target, array.size, gc_dict.get(target, 0),
            target_seq_dict.get(target, ''),
            target_count_dict.get(target, 0),
            np.count_nonzero(array) / array.size if array.size > 0 else 0,
    ...
    # Write probe info
    probe_colnames = ['probe_id', 'gc', 'tm', 'num_targets', 'sequence']
    probe_info = pd.DataFrame(columns=probe_colnames)
    for record in SeqIO.parse(probe_fasta, 'fasta'):
        gc = SeqUtils.gc_fraction(record.seq)
        tm_val = mt.Tm_NN(record.seq, nn_table=mt.R_DNA_NN1)
        info_list = [record.id, gc, tm_val,
                     probe_count_dict.get(record.id, 0), str(record.seq)]
...
def run_pipeline(args):
    ...
    with open(summary_path, "w") as out:
        ...
        if primer_seq:
            param_info["T7_primer"] = "GCTAATACGACTCACTATAGGG"
            param_info["amplification_primer"] = primer_seq
            param_info["oligo_length"] = (
                len("GCTAATACGACTCACTATAGGG") + args.bait_length +
    ...
    if not args.no_opool and primer_seq:
        oligo_len = len("GCTAATACGACTCACTATAGGG") + args.bait_length + len(str(Seq(primer_seq).reverse_complement()))
        lines += [
            "",
            "  OLIGO POOL SYNTHESIS",
            f"    T7 promoter:             GCTAATACGACTCACTATAGGG (21 nt)",
