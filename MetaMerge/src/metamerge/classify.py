    ...
    for row in tqdm(megan_df.to_dict(orient="records"), desc="Classifying taxa", unit="taxon"):
        ...
        if best_exact:
            record.update({
                ...
                "Holi_best_mean_L":              float(best_exact["mean_L"]) if pd.notna(best_exact.get("mean_L")) else np.nan,
                "Holi_best_std_L":               float(best_exact["std_L"])  if pd.notna(best_exact.get("std_L"))  else np.nan,
                "Holi_best_mean_GC":             float(best_exact["mean_GC"]) if pd.notna(best_exact.get("mean_GC")) else np.nan,
            ...
            record.update({
                ...
                "Holi_best_mean_L":              np.nan,
                "Holi_best_std_L":               np.nan,
                "Holi_best_mean_GC":             np.nan,
