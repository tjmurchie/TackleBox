    ...
    def test_taxon_present_only_in_fillet_still_appears(self, config):
        ...
        fillet = pd.concat([
            ...
            pd.DataFrame({
                ...
                "direct_hard_reads": [500.0], "eco_support": [True], "pal_support": [False], "fos_support": [False],
            }),
        ], ignore_index=True)
        ...
        # is capped at "Supported" per evidence._classify_single_source.
...
class TestValidation:
    ...
    def test_megan_without_holi_raises(self, config):
        megan = pd.DataFrame({
            "tax_id": [9696], "tax_name": ["Puma"], "tax_rank": ["genus"], "tax_id_str": ["9696"],
        })
        with pytest.raises(ValueError, match="holi_df is None"):
